    ...
    try {
        ...
        switch (versioningScheme) {
            ...
            case "byEnvVar":
                ...
                if (!version) {
                    tl.setResult(
                        ...
                        tl.loc("Error_NoValueFoundForEnvVar")
                    );
                    break;
                }
                break;
            case "byBuildNumber":
                ...
                if (tl.getVariable("SYSTEM_HOSTTYPE") === "release") {
                    tl.setResult(
                        tl.TaskResult.Failed,
                        tl.loc("Error_AutomaticallyVersionReleases")
                    );
                    return;
                }
                ...
                if (!versionMatches) {
                    tl.setResult(
                        tl.TaskResult.Failed,
                        tl.loc("Error_NoVersionFoundInBuildNumber")
                    );
                    return;
                }
                ...
                if (versionMatches.length > 1) {
                    tl.warning(
                        tl.loc("Warning_MoreThanOneVersionInBuildNumber")
                    );
                }
        ...
        if (!searchPatternInput) {
            ...
            if (!useLegacyFind) {
                ...
                let searchPatterns: string[] = nutil.getPatternsArrayFromInput(
                    searchPatternInput
                );
                filesList = tl.findMatch(
                    undefined,
                    searchPatterns,
                    findOptions,
                    matchOptions
        ...
        if (!filesList || !filesList.length) {
            tl.setResult(
                tl.TaskResult.Failed,
                tl.loc("Info_NoFilesMatchedTheSearchPattern")
            );
            return;
        }
